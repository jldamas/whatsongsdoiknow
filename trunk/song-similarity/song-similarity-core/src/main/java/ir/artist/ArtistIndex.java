 ...
 * 
 */
public class ArtistIndex extends DocumentIndex
{
	public ArtistIndex() { }
	
	public ArtistIndex(String input, String dic, String inv, String docPos)
	{
			...
			if (e != null)
			{
				for (Integer docId : e.getDocPositions().keySet())
				{
					if (!usedIds.contains(docId))
					{
						...
						ArtistReport dr = new ArtistReport(docId, inputFileName, documentLoc, this, q);
						ret.add(dr);
					}
				}
			}
		...
		try
		{
			...
			while (line != null)
			{
				...
				if (m.matches())
				{
					...
					currArtistName = m.group(6);
					documentLocations.put(currDocId, new DocInfo(previousLineStart, new HashSet<String>()));
					termPosition = 1;

					currArtistName = normalizeLine(currArtistName);

					StringTokenizer st = new StringTokenizer(currArtistName);
